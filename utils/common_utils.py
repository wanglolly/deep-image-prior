...
def crop_npimage(img, d=32):
    ...
    bbox = [
            int((img.shape[2] - new_size[0])/2), 
            int((img.shape[3] - new_size[1])/2),
            int((img.shape[2] + new_size[0])/2) - 1,
            int((img.shape[3] + new_size[1])/2) - 1,
    ]

    img_cropped = img[0,1][0,2][bbox[0], bbox[2]][bbox[1], bbox[3]]
    return img_cropped
...
def saveImage(path, images_np, nrow =8, factor=1, interpolation=None):
